...
class AppSettingsService:
    ...
    def validate_git_sync(self, source: GitShortcutSyncSource | None = None) -> GitShortcutSyncValidation:
        ...
        repo_payload = self._github_json(f"/repos/{repo_ref.owner}/{repo_ref.repo}")
        ...
        if requested_paths:
            ...
            for path in requested_paths:
                candidate = self._validate_file_candidate(repo_ref, branch, path)
                if candidate:
        ...
        tree = self._github_json(f"/repos/{repo_ref.owner}/{repo_ref.repo}/git/trees/{urllib.parse.quote(branch, safe='')}?recursive=1")
        files = [
            candidate
            for candidate_path in self._candidate_paths(tree)
            if (candidate := self._validate_file_candidate(repo_ref, branch, candidate_path)) is not None
        ]
        if files:
            return self._validation_success(source, repo_ref, branch, files)
    ...
    def _sync_source(self, source: GitShortcutSyncSource) -> GitShortcutSyncSourceResult:
        ...
        for file in validation.files:
            ...
            if target.exists() and next_shas.get(file.file_path) == file.file_sha:
                continue
            content = self._download_match_file(repo_ref, branch, file.file_path)
    ...
    def _validate_file_candidate(self, repo_ref: GitHubRepoRef, branch: str, file_path: str) -> GitShortcutSyncFile | None:
        try:
        ...
        if not download_url:
            return None
        try:
            content = self._download_url(download_url)
    ...
    def _download_match_file(self, repo_ref: GitHubRepoRef, branch: str, file_path: str) -> str:
        content_payload = self._github_json(
            f"/repos/{repo_ref.owner}/{repo_ref.repo}/contents/{urllib.parse.quote(file_path, safe='/')}?ref={urllib.parse.quote(branch, safe='')}"
        )
        ...
        return self._download_url(download_url)
    ...
    def _github_json(self, path: str) -> Any:
        ...
        request = urllib.request.Request(url, headers={"Accept": "application/vnd.github+json", "User-Agent": "EspansoEdit"})
        try:
            with urllib.request.urlopen(request, timeout=10) as response:
                return json.loads(response.read().decode("utf-8"))
        except urllib.error.HTTPError as exc:
            if exc.code == 404:
                raise AppError("GITHUB_NOT_FOUND", "GitHub repository or file was not found.", status_code=404) from exc
    ...
    def _download_url(self, url: str) -> str:
        request = urllib.request.Request(url, headers={"User-Agent": "EspansoEdit"})
        try:
            with urllib.request.urlopen(request, timeout=10) as response:
